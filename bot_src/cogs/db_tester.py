...
class DBTestCog(commands.Cog):
    ...
    @app_commands.command(name="delete_user", description="Delete a user from the database")
    ...
    async def delete_user(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if user:
            await interaction.response.send_message(f"User {member.display_name} deleted from the database.")
        else:
            await interaction.response.send_message(f"User {member.display_name} not found in the database.")
    ...
    async def update_user(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if user:
            await interaction.response.send_message(f"User {member.display_name} already exists in the database.")
        else:
            new_user = DBUtils.update_point(member_id)
            await interaction.response.send_message(f"User {member.display_name} added to the database with ID {new_user.discord_id}.")

    @app_commands.command(name="add_points", description="Add points to a user")
    @app_commands.describe(member="The Discord ID of the user", exp="Experience points to add", level="Level to set")
    async def add_points(self, interaction: discord.Interaction, member: discord.Member, exp: int, level: int):
        member_id = member.id
        user = DBUtils.get_user(member_id)
        if not user:
            user = DBUtils.update_point(member_id)
    ...
    @app_commands.command(name="get_user_info", description="Get user info from the database")
